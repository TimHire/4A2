...
!     INSERT
      real, allocatable :: mass_flow_out(:), temp(:)
      real :: mach_lim, t_lim
      real, allocatable :: v_guess(:), ro_guess(:)
      
      do n=1,av%nn
      
      allocate(v_guess(g(n)%ni)); allocate(ro_guess(g(n)%ni));
      allocate(mass_flow_out(g(n)%ni)); allocate(temp(g(n)%ni));
      allocate(l_i(g(n)%ni))
...
!         Store the exit density and internal energy as if they were uniform 
          g(n)%ro = ro_out 
          g(n)%roe  = g(n)%ro * (av%cv * t_out + 0.5 * v_out**2)
...
!         INSERT
          mass_flow_out = ro_out * v_out * l_i(ni)
...
!         INSERT 
          do j = 1, nj
             do i = 1,ni-1
                 ...
                 g(n)%roe(i,j)  = g(n)%ro(i,j) * (av%cv * temp(i) + 0.5 * v_guess(i)**2 )
             end do
          end do 
...
!         INSERT
          ...
          write(6,*) 'ro =', g(n)%ro(1,1), 'rovx =', g(n)%rovx(1,1), 'rovy =', g(n)%rovy(1,1), 'roe =', g(n)%roe(1,1)
...
!     useful as a reference to non-dimensionalise the convergence
      ...
      deallocate(l_i,v_guess,ro_guess,mass_flow_out,temp)
