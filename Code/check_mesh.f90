...
!     Consider the absolute difference in the lengths of the x and y sides separately
      x_error = maxval(abs(g(n)%lx_i(1:ni-1,1:nj-1) + g(n)%lx_j(1:ni-1,1:nj-1) - &
      g(n)%lx_i(2:ni,1:nj-1) - g(n)%lx_j(1:ni-1,2:nj)))
      y_error = maxval(abs(g(n)%ly_i(1:ni-1,1:nj-1) + g(n)%ly_j(1:ni-1,1:nj-1) - &
      g(n)%ly_i(2:ni,1:nj-1) - g(n)%ly_j(1:ni-1,2:nj)))
